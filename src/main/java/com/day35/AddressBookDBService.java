...
public class AddressBookDBService {
	...
	private List<AddressBookData> getAddressBookData(ResultSet resultSet) {
		...
		try {
			while (resultSet.next()) {
				...
				addressBookList
						.add(new AddressBookData(typeId, firstName, lastName, phoneNumber, email, city, state, zip));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public int updateAddressBookRecord(String name, String phoneNumber) throws AddressBookException {
		String query = String.format("update addressbook set phoneNumber = '%s' where firstName= '%s' ;", phoneNumber,
				name);
		try (Connection connection = this.getConnection()) {
